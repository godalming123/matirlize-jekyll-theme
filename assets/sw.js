...
const loadAndAddToCache = (request, cache) =>
  fetch(request).then((response) => {
    ...
    return response || getFromCache(cache, "fallback_offline", "We really are'nt able to get that right now!") ;//return the response or the fallback offline page if that returns a 404
  });
